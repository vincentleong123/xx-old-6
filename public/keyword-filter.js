...
class KeywordFilter {
   ...
   */
  renderKeywordGrid(keywords, isLoadMore = false) {
    if (!this.frequencyGrid) return;
    ...
    if (isLoadMore) {
      keywords.forEach((item) => {
        ...
        const freqSize = Math.min(1.4, 0.8 + (item.frequencyPercentage / 100) * 0.6); // 0.8-1.4em based on freq
        pill.style.fontSize = `${freqSize}em`;
      ...
      keywords.forEach((item) => {
        ...
        const freqSize = Math.min(1.4, 0.8 + (item.frequencyPercentage / 100) * 0.6);
        pill.style.fontSize = `${freqSize}em`;
      ...
      // Add Load More button if more keywords available
      if (keywords.length < this.allKeywords.length) {
        const loadMoreBtn = document.createElement('div');
        loadMoreBtn.className = 'keyword-pill load-more';
        loadMoreBtn.innerHTML = `
          <div class="keyword-name">Load More Keywords</div>
          <div class="keyword-count">${this.allKeywords.length - keywords.length} more</div>
        `;
        loadMoreBtn.addEventListener('click', () => this.loadMoreKeywords());
        this.frequencyGrid.appendChild(loadMoreBtn);
      }
    }
  }

  async loadMoreKeywords() {
    // Load next batch of 50 keywords
    const currentLoaded = this.frequencyGrid.children.length - 1; // -1 for load more button
    const nextBatch = this.allKeywords.slice(currentLoaded, currentLoaded + 50);
    
    this.renderKeywordGrid(nextBatch, true);
  }
